...
public class VarComputer {
	...
	public VarComputer(Problem p, CloudComputer computer, CloudProcess[] processes,
		...
		p.post(p.scalProd(requiredMemories, processVars),"<=",computer.getMemory());
		p.post(p.scalProd(requiredCpuPowers, processVars),"<=",computer.getCpuPower());
		p.post(p.scalProd(requiredNetworkBandwidths, processVars),"<=",computer.getNetworkBandwidth());
	}
