...
public class Formula
{
  public static void main(String[] args) {
    try {
      ...
      // define variables
      IntVar X = c.addIntVar(0, 20000, "X");
      IntVar Y = c.addIntVar(0, 50000, "Y");
      System.out.println("vars created!");

      // add "formula" constraints
//      c.addConstraint("X**2 + Y**2 < 20"); // in symbolic forms
      c.addConstraint(X.mul(X).add(Y.mul(Y)).lt(200)); // in direct form

//      c.addConstraint("X + Y == 5"); // in symbolic forms
      c.addConstraint(X.add(Y).eq(10)); // in direct form
