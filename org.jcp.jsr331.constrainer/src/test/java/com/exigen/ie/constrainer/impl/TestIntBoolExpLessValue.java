...
public class TestIntBoolExpLessValue extends TestCase {
  ...
  public void test1(){
    ...
    try{
      ...
      System.out.println("One: " + intvar.toString());
  ...
  public void test2(){
		IntVar intvar = C.addIntVar(0, 10);
		C.addConstraint(intvar.lt(8).asConstraint());
		C.addConstraint(intvar.ge(0).asConstraint());
		// test execute
		try {
			C.postConstraints();
			System.out.println("Two: " + intvar.toString());
			assertEquals(8 - 1, intvar.max());
			assertEquals(0, intvar.min());
		} catch (Failure f) {
			fail("test failed");
