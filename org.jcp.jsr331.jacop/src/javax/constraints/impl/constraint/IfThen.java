...
public class IfThen extends Constraint {
	...
	public IfThen(javax.constraints.Constraint c1, javax.constraints.Constraint c2) {
		...
		if (!(c2 instanceof PrimitiveConstraint))
			throw new RuntimeException("Constraint " + c2 + 
					" has no implementation for the method implies." +
			" It cannot be used in logical expressions.");

		JaCoP.constraints.Constraint[] array = new JaCoP.constraints.Constraint[2];
		array[0] = new JaCoP.constraints.IfThen((PrimitiveConstraint)c1.getImpl(), (PrimitiveConstraint)c2.getImpl());
