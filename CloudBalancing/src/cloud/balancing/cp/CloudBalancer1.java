	...
	static public void balance(CloudComputer[] computers, CloudProcess[] processes) {
		...
		try {
			for (int i = 0; i < computers.length; i++) {
				...
				Var[] computerCostVars = new Var[processes.length];
				for (int j = 0; j < processes.length; j++) {
					CloudProcess process = processes[j];
					computerCostVars[j] = p.variable("cost"+i+"-"+j,0,computer.getCost());
					...
					Constraint costMax = p.linear(computerCostVars[j],"=",computer.getCost());
					p.post(c1.implies(c2.and(c3).and(c4).and(costMax)));
					...
					Constraint costZero = p.linear(computerCostVars[j],"=",0);
					p.post(c5.implies(c6.and(c7).and(c8).and(costZero)));
				}
				...
				costVars[i] = p.sum(computerCostVars);
			}
...
//		String test = "./data/cb-0800comp-2400proc.xml";
		...
		CloudBalancer1.balance(computers,processes);
