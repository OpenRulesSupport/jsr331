...
public class PrintSchedule implements Constants {
	...
	class TaskScheduleComparator<T> implements Comparator<T> {
		...
		public int compare(T o1, T o2) {
			return ((((ArrayList<Variable>) o1).get(1)).min() * 1000 + 
					(((ArrayList<Variable>) o1).get(3)).min())
					- (( ((ArrayList<Variable>) o2).get(1)).min() * 1000 +
					  ( ((ArrayList<Variable>) o2).get(3)).min());
		}
	}
	 ...
	 */
	public PrintSchedule(String[] name, Variable[] t, int[] d, Variable[] r) {
	 ...
	 * @param r resource usage of the operations.
	 */
	public PrintSchedule(String[] name, Variable[] t, Variable[] d, Variable[] r) {
		n = new ArrayList<String>();
		this.t = t;
		this.r = r;
	...
	public PrintSchedule(ArrayList<String> name, 
						ArrayList<Variable> t,
						ArrayList<Integer> d, 
						ArrayList<Variable> r) {
		n = name;
		this.t = new Variable[t.size()];
		for (int i = 0; i < t.size(); i++)
			this.t[i] = t.get(i);
		this.r = new Variable[r.size()];
	 ...
	 */
	public PrintSchedule(ArrayList<String> name, 
						ArrayList<? extends Variable> t,
						int[] d, 
						ArrayList<? extends Variable> r) {
		n = name;
		this.t = new Variable[t.size()];
		for (int i = 0; i < t.size(); i++)
			this.t[i] = t.get(i);
	...
	public String toString() {
		...
		//	s = s + i + "\t";
			int j = 0;
			// int k = 1;
			int start = ((Variable) TaskArr[j].get(1)).min();
			...
			while (start <= i && j < TaskArr.length) {
				int res = ((Variable) TaskArr[j].get(3)).min();
				start = ((Variable) TaskArr[j].get(1)).min();
