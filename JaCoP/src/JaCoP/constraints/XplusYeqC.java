...
public class XplusYeqC extends PrimitiveConstraint implements Constants {

	static int IdNumber = 1;

	final static short type = addeqC;
	 ...
	 * @param c constant c.
	 */
	public XplusYeqC(Variable x, Variable y, int c) {
		numberId = IdNumber++;
		numberArgs = 2;
		X = x;
		Y = y;
	...
	public void consistency(Store store) {
		while (store.newPropagation) {
			...
			for (int i = xDom.noIntervals() - 1; i >= 0; i--)
				yDomIn.addDom(new Interval(C - xDom.rightElement(i), C
						- xDom.leftElement(i)));
			...
			for (int i = yDom.noIntervals() - 1; i >= 0; i--)
				xDomIn.addDom(new Interval(C - yDom.rightElement(i), C
						- yDom.leftElement(i)));
	...
	public org.jdom.Element getPredicateDescriptionXML() {
		...
		predicate.setAttribute("name", Constants.id_addeqC + String.valueOf(C));
		...
		functional.setText("eq(add(" + X.id() + "," + Y.id() + "),"
				+ String.valueOf(C) + ")");
	...
	public void notConsistency(Store store) {
		while (store.newPropagation) {
			...
			if (X.singleton()) {
				...
				// store.in(Y, Domain.domain.complement(C - X.value()));
			} else if (Y.singleton()) {
				X.domain.inComplement(store.level, X, C - Y.value());
				// store.in(X, Domain.domain.complement(C - Y.value()));
			}
		}
	}
	...
	public org.jdom.Element toXML() {
		...
		constraint.setAttribute("reference", Constants.id_addeqC
				+ String.valueOf(C));
