...
public class XplusClteqZ extends PrimitiveConstraint implements Constants {

	static int IdNumber = 1;

	final static short type = addCltEq;
	 ...
	 * @param z variable z.
	 */
	public XplusClteqZ(Variable x, int c, Variable z) {
		numberId = IdNumber++;
		numberArgs = 3;
		X = x;
		C = c;
	...
	public void consistency(Store store) {
		...
		// store.in(X, MinInt, Z.max() - C);
		X.domain.inMax(store.level, X, Z.max() - C);
		// store.in(Z, X.min() + C, MaxInt);
	...
	public org.jdom.Element getPredicateDescriptionXML() {
		...
		predicate.setAttribute("name", Constants.id_addCle + String.valueOf(C));

		org.jdom.Element parameters = new org.jdom.Element("parameters");
		parameters.setText("int " + X.id() + " int " + Z.id());
		...
		functional.setText("le(add(" + X.id() + ","
	...
	public void notConsistency(Store store) {
		store.newPropagation = false;
		X.domain.inMin(store.level, X, Z.min() - C);
	...
	public org.jdom.Element toXML() {
		...
		constraint.setAttribute("reference", Constants.id_addCle + String.valueOf(C));
