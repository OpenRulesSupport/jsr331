...
public class XgteqY extends PrimitiveConstraint implements Constants {

	static int IdNumber = 1;

	final static short type = gtEq;
	 ...
	 * @param y variable y.
	 */
	public XgteqY(Variable x, Variable y) {
		numberId = IdNumber++;
		numberArgs = 2;
		X = x;
		Y = y;
	...
	public void consistency(Store store) {
		...
		// store.in(X, Y.min(), MaxInt);
		Y.domain.inMax(store.level, Y, X.max());
		// store.in(Y, MinInt, X.max());
		// }
	}
	...
	public org.jdom.Element getPredicateDescriptionXML() {
		...
		predicate.setAttribute("name", Constants.id_gtEq);
		...
		functional.setText("ge(" + X.id() + "," + Y.id() + ")");
	...
	public org.jdom.Element toXML() {
		...
		constraint.setAttribute("reference", Constants.id_gtEq);
