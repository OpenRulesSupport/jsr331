...
public class ExtensionalSupportSTR extends Constraint implements Constants {

	final static short type = extensionalSupportSTR;
	 ...
	 */
	public HashMap<Variable, Integer> varToIndex;
	 ...
	 */
	public ExtensionalSupportSTR(Variable[] variables, int[][] tuples, 
		...
		views = new IndexDomainView[x.length];
	...
	public void consistency(Store store) {
		...
		if (firstConsistencyCheck) {
			...
			valuesInFocus = new IntervalDomain[x.length];
			...
			for (int[] t : tuples) {
				...
				if (stillSupport[i]) {
					...
					int m = 0;
					for (int val : t) {
						valuesInFocus[m].addDom(val, val);
						m++;
					}
			...
			domainSizeAfterConsistency = new int[x.length];

			for (int j = 0; j < views.length; j++) {

				x[j].domain.in(store.level, x[j], valuesInFocus[j]);
				
				views[j] = new IndexDomainView(x[j], true);
			}
		...
		if (backtrackOccured) {
			...
			for (int i = 0; i < x.length; i++)
				...
				if (domainSizeAfterConsistency[i] != 0)
					domainSizeAfterConsistency[i] = x[i].getSize();
		}
		...
		for (int i = 0; i < x.length; i++) {
			if (x[i].getSize() != domainSizeAfterConsistency[i]) {
			...
			if (domainSizeAfterConsistency[i] != 1) {
				...
				nbGlobalValuesToBeSupported += x[i].getSize();
				nbValuesToBeSupported[i] = x[i].getSize();
			}
		...
		if (lastAssignedVariablePosition != -1) 
			lastAssignedIndex = views[lastAssignedVariablePosition].indexOfValue(
					x[lastAssignedVariablePosition].value() );
		...
		for (int i = 0; i < nbSupportsVariables; i++) {
			int position = supportsVariablePositions[i];		
			if ( nbValuesToBeSupported[position] == x[position].getSize() )
				throw Store.failException;
		}
...
//		 If consistency function mode
	...
	public String id() {
		if (id != null)
			...
			return id_extensionalSupportSTR + numberId;
		}
	...
	public void impose(Store store) {
		...
		varToIndex = new HashMap<Variable, Integer>();
		...
		nbValuesToBeSupported = new int[x.length];		
		validityVariablePositions = new int[x.length];
		supportsVariablePositions = new int[x.length];
	...
	public org.jdom.Element toXML() {
		...
		relation.setAttribute("name", id() );
		relation.setAttribute("arity", String.valueOf(x.length) );
		
		relation.setAttribute("semantics", "supports");
		...
		relation.setAttribute("nbTuples", String.valueOf( noTuples ) );		
		relation.setText(tupleString.toString());
