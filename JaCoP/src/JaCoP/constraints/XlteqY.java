...
public class XlteqY extends PrimitiveConstraint implements Constants {

	static int IdNumber = 1;

	final static short type = ltEq;
	 ...
	 * @param y variable y.
	 */
	public XlteqY(Variable x, Variable y) {
		numberId = IdNumber++;
		numberArgs = 2;
		X = x;
		Y = y;
	...
	public void consistency(Store store) {
		...
		// store.in(X, MinInt, Y.max());
		Y.domain.inMin(store.level, Y, X.min());
		// store.in(Y, X.min(), MaxInt);
		// }
	}
	...
	public org.jdom.Element getPredicateDescriptionXML() {
		...
		predicate.setAttribute("name", Constants.id_ltEq);
		...
		functional.setText("le(" + X.id() + "," + Y.id() + ")");
	...
	public org.jdom.Element toXML() {
		...
		constraint.setAttribute("reference", Constants.id_ltEq);
