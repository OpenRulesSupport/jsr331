...
public class XgtY extends PrimitiveConstraint implements Constants {

	static int IdNumber = 1;

	final static short type = gt;
	 ...
	 * @param y variable y.
	 */
	public XgtY(Variable x, Variable y) {
		numberId = IdNumber++;
		numberArgs = 2;
		X = x;
		Y = y;
	...
	public void consistency(Store store) {
		...
		// store.in(X, Y.min() + 1, MaxInt);
		Y.domain.inMax(store.level, Y, X.max() - 1);
		// store.in(Y, MinInt, X.max() - 1);
		// }
	}
	...
	public org.jdom.Element getPredicateDescriptionXML() {
		...
		predicate.setAttribute("name", Constants.id_gt);
		...
		functional.setText("gt(" + X.id() + "," + Y.id() + ")");
	...
	public org.jdom.Element toXML() {
		...
		constraint.setAttribute("reference", Constants.id_gt);
