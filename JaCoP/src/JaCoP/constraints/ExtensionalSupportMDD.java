...
public class ExtensionalSupportMDD extends Constraint implements Constants {
	...
	final static short type = extensionalSupportMDD;
    ...
    @Override
    public void impose(Store s) {

	if (mdd == null) {
	    mdd = new MDD(vars, table);
	    views = mdd.views;
    	
	    G_no = new SparseSet(mdd.freePosition);
	    table = null;
	    vars = null;
	}

    	s.registerRemoveLevelListener(this);
		
		for (int i = 0; i < mdd.vars.length; i++)
			 mdd.vars[i].putConstraint(this);

		this.G_no_size = new TimeStamp<Integer>(s, 0);
			
		s.addChanged(this);
		s.countConstraint();
		
		s.raiseLevelBeforeConsistency = true;

		if (mdd.freePosition > s.sparseSetSize)
			s.sparseSetSize = mdd.freePosition;
     ...
     */
    public ExtensionalSupportMDD(Variable [] vars, int [][] table) {
...
//		 If consistency function mode
	...
	public String id() {
		if (id != null)
			...
			return id_extensionalSupportMDD + numberId;
	}
	...
	public Element toXML() {
		...
		constraint.setAttribute("reference", id_extensionalSupportMDD);
		constraint.setAttribute("arity", String.valueOf(mdd.vars.length));

		
		StringBuffer scope = new StringBuffer();
		for (int i = 0; i < mdd.vars.length; i++)
			scope.append( mdd.vars[i].id()).append(" ");

		constraint.setAttribute("scope", scope.substring(0, scope.length() - 1));			
		
		constraint.addContent(mdd.toXML());
	...
	/**
	 * It creates constraint from XCSP description.
	 * @param constraint an XML description of the constraint.
	 * @param store constraint store in which context is being created.
	...
	static public Constraint fromXML(Element constraint, Store store) {
		...
		return new ExtensionalSupportMDD(new MDD(vars, constraint.getChild("mdd")));
