...
public class Knapsack extends Constraint implements Constants {
	 ...
	 */
	public HashMap<Variable, TreeLeaf> variableLeafMapping;
	 ...
	 */
	public final Variable knapsackCapacity;
	
	/**
	 * This is a finite domain variable to specify the knapsack profit. 
	 */
	public final Variable knapsackProfit;
	...
	public Knapsack(int[] profits, 
				    int[] weights, 
					Variable[] quantity,
					Variable knapsackCapacity, 
					Variable knapsackProfit) {
		...
		if (parent.getWMax() > availableCapacity) {
			...
			{ /* left part */
				...
				if (!current.isLeaf()) {
					...
					Variable quantity = ((TreeLeaf) current).quantity;
			...
			{ /* right part */
				...
				if (!current.isLeaf()) {
					...
					Variable quantity = leaf.quantity;
	...
	public void computeForbidden() {
		...
		while (true) {
			...
			if (intrusionWeight < itemMaxWeight) {
				...
				Variable quantity = leaf.getVariable();
	...
	public void impose(Store store) {
		...
		Variable zero = new Variable(store, 0, 0);
		
		leaves = new TreeLeaf[items.length];

		variableLeafMapping = new HashMap<Variable, TreeLeaf>();
	...
	public ArrayList<Variable> arguments() {
		...
		ArrayList<Variable> variables = new ArrayList<Variable>(items.length + 2);
	...
	public Element toXML() {
		...
		constraint.setAttribute("reference", id_knapsack);

		HashSet<Variable> scopeVars = new HashSet<Variable>();

		scopeVars.add(knapsackProfit);
		scopeVars.add(knapsackCapacity);
		
		for (int i = 0; i < items.length; i++)
			scopeVars.add(items[i].quantity);
		...
		org.jdom.Element term = new org.jdom.Element("knapsackCapacity");
		term.setText(knapsackCapacity.id());
		constraint.addContent(term);

		term = new org.jdom.Element("knapsackProfit");
		term.setText(knapsackProfit.id());
		constraint.addContent(term);

		org.jdom.Element tList = new org.jdom.Element("quantity");

		StringBuffer tString = new StringBuffer();
		for (int i = 0; i < items.length; i++)
			tString.append( items[i].quantity.id() ).append( " " );

		tList.setText(tString.toString().trim());

		constraint.addContent(tList);
				
		tList = new org.jdom.Element("weights");
		tString = new StringBuffer();
		for (int i = 0; i < items.length; i++)
			tString.append( String.valueOf( items[i].weight ) ).append( " " );

		tList.setText(tString.toString().trim());

		constraint.addContent(tList);

		tList = new org.jdom.Element("profits");
		tString = new StringBuffer();
		for (int i = 0; i < items.length; i++)
			tString.append( String.valueOf( items[i].profit ) ).append( " " );
