...
public class TimeStamp<T> {
	...
	private int pointer4Last = -1;

	private transient int[] stamps = new int[10];

	Store store;

	@SuppressWarnings("unchecked")
	private transient T[] values = (T[]) new Object[10];
	 ...
	 * 
	 * @param stamp the number of the level. 
	 */
	public void remove(int stamp) {

		while (pointer4Last >= 0 && stamps[pointer4Last] >= stamp) {
			
			if (Switches.trace)
				if (Switches.traceStore)
					if (Switches.traceLevelRemoval)
						System.out.println("Remove level " + stamps[pointer4Last]
						                   + "timestamp" + this);
	...
	public void update(T val) {
		...
		if (stamps[pointer4Last] == store.level) {
			...
			assert (false) : "Error - Timestamp" + this	+ "has greater level than store " 
							 + "- missing remove";
		}
