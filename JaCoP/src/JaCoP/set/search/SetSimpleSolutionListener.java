/**
 *  SetSimpleSolutionListener.java 
...
public class SetSimpleSolutionListener extends SimpleSolutionListener {
	...
	/**
	 * @return the 2D array containing all solutions.
	 */
	public Set[][] getSetSolutions() {
	 ...
	 * @return the solution with a given number.
	 */

	public Set[] getSetSolution(int no) {
	...
	public void recordSolution() {
		...
		if (recordSolutions) {
			...
			if (noSolutions >= solutions.length) {
				...
				Set[][] oldSolutions = solutions;
				solutions = new Set[noSolutions * 2][];
			...
			Set[] currentSolution = new Set[vars.length];

			for (int i = 0; i < vars.length; i++){
				if(vars[i].dom().domainID()==SetDomain.SetDomainID){
					SetDomain sd = (SetDomain) vars[i].dom();
					currentSolution[i] = sd.glb();
				}
			}
	...
	public boolean executeAfterSolution(Search search, SelectChoicePoint select) {
		...
		if (vars == null) {
			...
			solutions = new Set[1][vars.length];
	...
	public boolean assignSolution(Store store, int number) {
		...
		if (vars != null) {
			...
			for (int i = 0; i < vars.length; i++)
				if(vars[i].domain.domainID() == SetDomain.SetDomainID){
					SetDomain sd = (SetDomain) vars[i].domain;
					sd.in(store.level, vars[i], solutions[number][i],
							solutions[number][i]);
				}
	...
	public PrimitiveConstraint[] returnSolution(int number) {
		...
		if (vars != null) {
			...
			for (int i = 0; i < vars.length; i++) {
				result[no] = new XeqY(vars[i], new Variable(vars[i].store,"",solutions[i][number]));
				no++;
			}
