...
public class SetSimpleSelect implements SelectChoicePoint {
	...
	Variable[] searchVariables;

	ComparatorVariable variableOrdering;

	ComparatorVariable tieBreakingComparator = null;

	Indomain valueOrdering;
	...
	public IdentityHashMap<Variable, Integer> position;
	 ...
	 */
	public SetSimpleSelect(Variable[] variables, ComparatorVariable varSelect,
			Indomain indomain) {

		position = new IdentityHashMap<Variable, Integer>();
		...
		this.searchVariables = new Variable[position.size()];

		for (Iterator<Map.Entry<Variable, Integer>> itr = position.entrySet()
				.iterator(); itr.hasNext();) {
			Map.Entry<Variable, Integer> e = itr.next();
	 ...
	 * @param varSelect the variable comparator to choose the variable.
	 * @param indomain the value heuristic to choose a value for a given variable.
	 * @param eInS_leftTree boolean variable, if true then eInS is used for the left side of the searchtree.
	 */
	public SetSimpleSelect(Variable[] variables, ComparatorVariable varSelect,
			Indomain indomain, boolean eInS_leftTree) {
	 ...
	 */
	public SetSimpleSelect(Variable[] variables, ComparatorVariable varSelect,
			ComparatorVariable tieBreakerVarSelect, Indomain indomain) {
	 ...
	 */
	public SetSimpleSelect(Variable[] variables, ComparatorVariable varSelect,
			ComparatorVariable tieBreakerVarSelect, Indomain indomain, boolean eInS_leftTree) {
	...
	/**
	 * It always returns null as choice point is obtained by getChoiceConstraint.
	 */
	...
	public int getChoiceValue() {
		...
		assert (searchVariables[currentIndex].domain != null);

		return valueOrdering.indomain(searchVariables[currentIndex].domain);
	...
	public PrimitiveConstraint getChoiceConstraint(int index) {
		...
		//Set constraint.
		Variable v = searchVariables[index];
		if(!v.dom().singleton()){
			SetDomain ss = (SetDomain)v.dom();
			Set s = ss.lub().subtract(ss.glb());
			int e = valueOrdering.indomain(s); 
			PrimitiveConstraint c = new EInS(e,v);
	...
	public IdentityHashMap<Variable, Integer> getVariablesMapping() {
	...
	public Variable placeSearchVariable(int searchPosition, int variablePosition) {

		if (searchPosition != variablePosition) {

			Variable temp = searchVariables[searchPosition];
