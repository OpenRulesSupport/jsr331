...
public class TransformExtensional implements InitializeListener {
	 ...
	 * listener.
	 */
	public ArrayList<Variable> variablesTransformationScope = new ArrayList<Variable>();
	...
	public void executedAtInitialize(Store store) {
		...
		SelectChoicePoint select = new SimpleSelect(variablesTransformationScope.toArray(new Variable[1]),
		...
		if (searchResult) {
			...
			for ( Variable v : variablesTransformationScope ) {
				...
				for (Constraint checkConstraint : constraintsInQuestion) {
					...
					for (Variable m : checkConstraint.arguments())
			...
			Variable[] vars = search.getSolutionListener().getVariables();
