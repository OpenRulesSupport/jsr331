...
public class SimpleMatrixSelect implements SelectChoicePoint {
	...
	ComparatorVariable mainComparator = null;
...
//	int subListSize;
	...
	public IdentityHashMap<Variable, Integer> position = new IdentityHashMap<Variable, Integer>();

	/**
	 * It stores variables which need to be labelled.
	 */
	public ArrayList<ArrayList<Variable>> searchVariables = new ArrayList<ArrayList<Variable>>();
	...
	public SimpleMatrixSelect(Variable[][] vars, Indomain indomain) {
	 ...
	 */
	public SimpleMatrixSelect(Variable[][] vars,
			ComparatorVariable mainComparator, Indomain indomain) {
	 ...
	 */
	public SimpleMatrixSelect(Variable[][] vars,
			ComparatorVariable mainComparator,
			ComparatorVariable tieBreakingComparator, Indomain indomain) {
	...
	public SimpleMatrixSelect(Variable[][] vars,
			ComparatorVariable mainComparator,
			ComparatorVariable tieBreakingComparator, 
			Indomain indomain,
			int pivotPosition) {
		...
		for (int i = 0; i < vars.length; i++) {
			...
			ArrayList<Variable> current = new ArrayList<Variable>();
	...
	public Variable getChoiceVariable(int firstVariable) {
		...
		if (mainComparator == null) {
			...
			while (firstVariable < finalIndex) {
				...
				ArrayList<Variable> row = searchVariables.get(firstVariable);
		...
		Variable currentVariable = searchVariables.get(firstVariable).get(
				pivotPosition);
		...
		for (int currentPosition = firstVariable + 1; currentPosition < finalIndex; currentPosition++) {
			...
			if (v.singleton()) {
				...
				ArrayList<Variable> row = searchVariables.get(currentPosition);
		...
		if (optimalPosition != firstVariable) {
			ArrayList<Variable> row = searchVariables.get(optimalPosition);
			// switch rows.
		...
		ArrayList<Variable> row = searchVariables.get(primaryIndex);
		for (int i = 0; i < row.size(); i++)
			if (!row.get(i).singleton()) {
				secondaryIndex = i;
				break;
