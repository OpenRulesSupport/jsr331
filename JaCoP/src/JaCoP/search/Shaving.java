...
public class Shaving implements ExitChildListener, ConsistencyListener {
	 ...
	 */
	public boolean onlyVariablesOfFailedConstraint = false;

	/**
	 * It stores the variables of the last failed constraints.
	 */
	public HashSet<Variable> varsOfFailedConstraint = new HashSet<Variable>();
	...
	ArrayList<HashMap<Variable, LinkedHashSet<Integer> >> shavable = new ArrayList<HashMap<Variable, LinkedHashSet<Integer> >> ();

	HashMap<Variable, LinkedHashSet<Integer>> notShavable = new HashMap<Variable, LinkedHashSet<Integer>>();	
	...
	public void rightChild(Variable var, int value, boolean status) {
		...
		if (!status) {
			...
			if (quickShave && leftChildWrongDecision) {
				...
				HashMap<Variable, LinkedHashSet<Integer>> current = shavable.get(position);
	...
	public boolean executeAfterConsistency(boolean consistent) {
		...
		if (!consistent) {
			recentlyFailedConstraint = store.currentConstraint;
			depth++;
			return false;
		}

		// Speculate based on neighbours
		HashMap<Variable, LinkedHashSet<Integer>> shavableCurrent = new HashMap<Variable, LinkedHashSet<Integer>>();
		...
		while (last > current) {
			...
			HashMap<Variable, LinkedHashSet<Integer>> shavableNeighbour = shavable.get(current);
			
			for (Variable shaveVar : shavableNeighbour.keySet()) {
		...
		if (!leftChildShaving || leftChild)
			for (Constraint g : shavingConstraints) {
				...
				Variable shaveVar = g.getGuideVariable();
				...
				if (onlyVariablesOfFailedConstraint)
	...
	boolean checkIfShavable(Variable var, Integer val) {
