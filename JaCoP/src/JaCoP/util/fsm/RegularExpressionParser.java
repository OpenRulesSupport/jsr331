...
public class RegularExpressionParser {
	...
	public static void main(String args[]) {
		...
		try {
			...
			RegularExpressionParser p = new RegularExpressionParser(
					new StringReader("(1*.(0.(1+0)*.0)*)*"));
			
			Expression e = p.parse(false);
			// if (Regular.debugAll)
			System.out.println("Succesful parsing of \n" + e);
			FSM d = e.parseToFSM();
			// if (Regular.debugAll)
			System.out.println("Graph \n" + d);

			Store store = new Store();
			Variable x1 = new Variable(store, "x1", 0, 1);
			Variable x2 = new Variable(store, "x2", 0, 1);
			Variable x3 = new Variable(store, "x3", 0, 1);
			Variable x4 = new Variable(store, "x4", 0, 1);
			Variable x5 = new Variable(store, "x5", 0, 1);
			Variable x6 = new Variable(store, "x6", 0, 1);
			Variable[] x = { x1, x2, x3, x4, x5, x6 };
			store.impose(new Regular(d, x));

			SelectChoicePoint select = new SimpleSelect(x,
		...
		public FSM parseToFSM() {
			...
			for (Expression e : this.disj) {
				if (first) {
					...
					Domain dom = tmp.initState.transitions.iterator().next().domain;
