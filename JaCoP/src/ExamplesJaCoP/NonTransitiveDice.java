...
public class NonTransitiveDice extends Example {
	...
	public void model() {
		...
		FDV faces[] = new FDV[noDices * noSides];

		for (int i = 0; i < faces.length; i++) {
			// Create FDV for each face
			faces[i] = new FDV(store, "d" + (i / noSides + 1) + "f"	+ (i % noSides + 1),
					// minimal value
					1,
					// maximal value
					noNumbers);
		...
		FDV wins[][][] = new FDV[noDices][noSides][noSides];

		for (int i = 0; i < noDices; i++)
			for (int j = 0; j < noSides; j++)
				for (int m = 0; m < noSides; m++)
					wins[i][j][m] = new FDV(store, "win_D" + (i + 1) + "->"
		...
		FDV winningSum[] = new FDV[noDices];
		for (int i = 0; i < noDices; i++) {
			winningSum[i] = new FDV(store, "noWins-d" + (i + 1) + "->d"
					+ ((i + 2) % noDices), noSides * noSides / 2 + 1,
					noSides * noSides);

			FDV matrix[] = new FDV[noSides * noSides];
			for (int j = 0; j < noSides; j++)
				for (int m = 0; m < noSides; m++)
		...
		FDV minimumWinning = new FDV(store, "MinDominance", 0, noSides * noSides);

		store.impose(new Min(minimumWinning, winningSum));
		...
		// objective
		// of diff
		FDV diff = new FDV(store, "diff", currentBest, currentBest);
		...
		if (reuseOfNumbers) {
			...
			FDV sides_two_consequtive_dices[] = new FDV[noSides * 2];
		...
		FDV maxNo = new FDV(store, "maxNo", noSides * 2, noDices * noSides);
		store.impose(new Max(maxNo, faces));
	...
	public static void main(String args[]) {
		...
		int currentBest = noSides * noSides / 2;
