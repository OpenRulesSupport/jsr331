...
public class MasterClass extends Example {
	...
	public void model() {
		...
		// voice names
		String[] glos = { "mezzosoprano", "soprano", "bass", "tenor", "tenor" };
		...
		// people's surnames
		String[] nazwisko = { "Kisley", "Robinson", "Robinson", "Walker",
				"Urlich" };
		...
		FDV zglos[] = new FDV[5];
		FDV zimie[] = new FDV[5];
		FDV znazwisko[] = new FDV[5];

		// FDV creation
		for (int i = 0; i < 5; i++) {
			zglos[i] = new FDV(store, glos[i], 1, 5);
			znazwisko[i] = new FDV(store, nazwisko[i], 1, 5);
			zimie[i] = new FDV(store, imie[i], 1, 5);
