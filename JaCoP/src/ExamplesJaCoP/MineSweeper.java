...
public class MineSweeper extends Example {
        ...
        for(int i = 0; i < r; i++) {
            for(int j = 0; j < c; j++) {
                ...
                if (problem[i][j] > X) {
                    ...
                    ArrayList<FDV> lst = new ArrayList<FDV>();
                    for(int a = -1; a <= 1; a++) {
                        for(int b = -1; b <= 1; b++) {
                            if (i+a >= 0 && j+b >=  0 &&
                                i+a < r && j+b < c) {
    ...
    /**
     * @todo This search uses still buggy implementation of iterative solution listener.
     * @param recordSolutions specifies if the solutions should be recorded.
     * @return true if a solution was found, false otherwise.
     */
    public boolean searchOneByOne(boolean recordSolutions) {
		...
		if (result)
			...
			System.out.println("Failed to find next solution");

		System.out.println("\n\t*** Execution time = " + (T2 - T1) + " ms");

		if ( search.assignSolution() )
			System.out.println("Assigning last solution successful");
		else
			System.out.println("Problem with assigning last solution");
    ...
    */
    ...
    public static void main(String args[]) {
        ...
        minesweeper.model( );
        minesweeper.searchOneByOne( true );
