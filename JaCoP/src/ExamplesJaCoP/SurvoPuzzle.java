...
public class SurvoPuzzle extends Example {
    ...
    FDV[][] x;      // the solution
    FDV[] x_arr;    // x as an array, for alldifferent
	...
	public void model() {
        ...
        for(int i = 0; i < r; i++) {
            for(int j = 0; j < c; j++) {
                x[i][j] = new FDV(store, "x_" + i + "_" + j, 1, r*c);
                if (matrix[i][j] > 0) {
                    store.impose(new XeqC(x[i][j], matrix[i][j]));
                }
                x_arr[c*i+j] = new FDV(store, "xa_" + i + "_" + j, 1, r*c);
        ...
        //
        for(int i = 0; i < r; i++) {
            FDV r_sum = new FDV(store, "r_" + i, 1, r*c*r*c);
        ...
        //
        for(int j = 0; j < c; j++) { 
            ArrayList<FDV> cols = new ArrayList<FDV>();
            for(int i = 0; i < r; i++) {
                cols.add(x[i][j]);
            }
            FDV c_sum = new FDV(store, "c_" + j, 1, r*c*r*c);
     ...
     */
    public static void printMatrix(FDV[][] matrix, int rows, int cols) {
