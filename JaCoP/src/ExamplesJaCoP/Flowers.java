...
public class Flowers extends Example {
	...
	public void model() {
		...
		FDV wifeT[] = new FDV[4];
		FDV wifeD[] = new FDV[4];
		FDV husbandT[] = new FDV[4];
		FDV husbandD[] = new FDV[4];
		FDV flowerT[] = new FDV[4];
		FDV flowerD[] = new FDV[4];
		FDV occasionT[] = new FDV[4];
		FDV occasionD[] = new FDV[4];

		for (int i = 0; i < 4; i++) {
			// Days in February are from 1 to 28.
			husbandD[i] = new FDV(store, husbandDay[i], 1, 28);
			wifeD[i] = new FDV(store, wifeDay[i], 1, 28);
			occasionD[i] = new FDV(store, occasionDay[i], 1, 28);
			flowerD[i] = new FDV(store, flowerDay[i], 1, 28);
			// There are 4 weeks in February.
			husbandT[i] = new FDV(store, husbandWeek[i], 1, 4);
			wifeT[i] = new FDV(store, wifeWeek[i], 1, 4);
			occasionT[i] = new FDV(store, occasionWeek[i], 1, 4);
			flowerT[i] = new FDV(store, flowerWeek[i], 1, 4);
		}
