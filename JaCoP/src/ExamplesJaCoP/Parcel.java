 ...
 *
 */
public class Parcel extends Example {
	...
	public void model() {
		...
		FDV loads[] = new FDV[noCities];

		for (int i = 0; i < cities.length; i++) {
			cities[i] = new FDV(store, "cities[" + i + "]", 1, cities.length);
			loads[i] = new FDV(store, "nextLoad[" + i + "]", minLoad, maxLoad);
			costs[i] = new FDV(store, "costs[" + i + "]", 0, 1000);
			vars.add(costs[i]);
		}
		...
		for (int i = 0; i < cities.length; i++) {
			...
			FDV nextTown = new FDV(store, "nextTown[" + i + "]", 1,
					cities.length);
		...
		for (int i = 0; i < cities.length; i++) {
			FDV tripLoads[] = new FDV[i + 1];
			for (int j = 0; j <= i; j++)
				tripLoads[j] = loads[j];
			FDV partialLoad = new FDV(store, "partialLoad[0-" + i + "]", 0, 15);
