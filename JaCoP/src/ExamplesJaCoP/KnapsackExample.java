...
public class KnapsackExample extends Example {
	...
	public void model() {
		...
		for (int i = 0; i < quantity.length; i++) {
			quantity[i] = new FDV(store, "Quantity_" + names[i], 0, maxs[i]);
			vars.add(quantity[i]);
		}
		
		FDV profit = new FDV(store, "Profit", 0, 1000000);
		FDV weight = new FDV(store, "Weight", 0, 1000000);
	...
	public void modelBasic() {
		...
		for (int i = 0; i < quantity.length; i++) {
			quantity[i] = new FDV(store, "Quantity_" + names[i], 0, 1);
			vars.add(quantity[i]);
		}
