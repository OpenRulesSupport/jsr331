...
/**
 * 
 * @author Czarnojan Mariusz, Burka Krystian, and Radoslaw Szymanek
 *
 * "A Round of Golf"
...
public class Golf extends Example {
	...
	public void model() {
		...
		for (int i = 0; i < 4; i++) {
			...
			FN[i] = new FDV(store, FNNames[i]);
			FN[i].addDom(70, 80);
			FN[i].addDom(82, 85);

			LN[i] = new FDV(store, LNNames[i]);
			LN[i].addDom(70, 80);
			LN[i].addDom(82, 85);

			Jobs[i] = new FDV(store, JobsNames[i]);
			Jobs[i].addDom(70, 80);
			Jobs[i].addDom(82, 85);
		...
		for (int i = 0; i < 4; i++) {
			FDV el1 = new FDV(store, "i" + i + "LN", 1, 4);
			FDV el2 = new FDV(store, "i" + i + "Jobs", 1, 4);
