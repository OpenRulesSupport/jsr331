...
public class BlueberryMuffins extends Example {
	...
	public void model() {
		...
		Variable person[] = new Variable[4];
		Variable last[] = new Variable[4];
		Variable profession[] = new Variable[4];
		Variable muffins[] = new Variable[4];

		// All variables are created with domain 0..3. Variables from
		// different arrays with the same values denote the same person.

		for (int i = 0; i < 4; i++) {
			last[i] = new Variable(store, lastnames[i], 0, 3);
			profession[i] = new Variable(store, professionNames[i], 0, 3);
			muffins[i] = new Variable(store, muffinnumbers[i], 0, 3);
			person[i] = new Variable(store, firstnames[i], 0, 3);
		...
		Variable six = new Variable(store, "six", 6, 6);
		Variable I1 = new Variable(store, "temp1", 1, 4);
		Variable I2 = new Variable(store, "temp2", 1, 4);
		...
		Variable I3 = new Variable(store, "temp3", 1, 4);
		Variable I4 = new Variable(store, "temp4", 1, 4);
