...
public class DollarAndTicket extends Example {
	...
	public void model() {
		...
		FDV firstname[] = new FDV[5];
		FDV surname[] = new FDV[5];
		FDV team[] = new FDV[5];

		// Each variable has a domain being created by subsequent calls to
		// addDom function.
		for (int i = 0; i < 5; i++) {
			firstname[i] = new FDV(store, firstnames[i]);
			...
			surname[i] = new FDV(store, surnames[i]);
		...
		FDV girlIndex = new FDV(store, "girlIndex", 1, 3);
		// First three indexes of firstname denote girls name
		// (the ordering of names is important here).
		FDV girlFirstname[] = { firstname[0], firstname[1], firstname[2] };
