...
public class Golomb extends Example {
	 ...
	 */
	public ArrayList<FDV> subs = new ArrayList<FDV>();
	...
	public void model() {
		...
		FDV numbers[] = new FDV[noMarks];

		for (int i = 0; i < numbers.length; i++) {
			// Create FDV for each natural number
			numbers[i] = new FDV(store, "n" + new Integer(i), (i + 1) * i / 2,
					noMarks * noMarks);
		...
		for (int i = 1; i < numbers.length; i++) {
			...
			for (int j = 0; j < i; j++) {
				...
				FDV sub = new FDV(store, "c" + i + "_" + j, (i - j)
