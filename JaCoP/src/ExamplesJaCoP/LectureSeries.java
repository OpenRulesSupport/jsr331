...
public class LectureSeries extends Example {
	...
	public void model() {
		...
		FDV last[] = new FDV[5];
		FDV days[] = new FDV[5];
		FDV subjects[] = new FDV[5];

		// FDV's creation
		// FDV's with the same value from different groups are related.
		for (int i = 0; i < 5; i++) {
			first[i] = new FDV(store, firstNames[i], 1, 5);
			last[i] = new FDV(store, lastNames[i], 1, 5);
			days[i] = new FDV(store, daysNames[i], 1, 5);
			subjects[i] = new FDV(store, subjectsNames[i], 1, 5);
		...
		FDV dayIndex4Itakura = new FDV(store, "dayIndex4Itakura", 1, 5);
		store.impose(new Element(dayIndex4Itakura, days, last[iItakura]));

		FDV dayIndex4Study = new FDV(store, "dayIndex4Study", 1, 5);
		store.impose(new Element(dayIndex4Study, days, subjects[iStudy]));

		FDV one = new FDV(store, "1", 1, 1);
		...
		FDV dayIndex4Haller = new FDV(store, "dayIndex4Haller", 1, 5);
		store.impose(new Element(dayIndex4Haller, days, last[iHaller]));

		FDV dayIndex4Eddie = new FDV(store, "dayIndex4Eddie", 1, 5);
		...
		FDV dayIndex4Duane = new FDV(store, "dayIndex4Duane", 1, 5);
		store.impose(new Element(dayIndex4Duane, days, first[iDuane]));

		FDV dayIndex4Art = new FDV(store, "dayIndex4Art", 1, 5);
