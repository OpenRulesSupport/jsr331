...
/**
 * 
 * It is a simple newspaper reading job-shop like 
 * scheduling problem.
 * 
...
public class Newspaper extends Example {
	...
	public void model() {
		...
		Variable[] algy = new Variable[4];
		Variable[] bertie = new Variable[4];
		Variable[] charlie = new Variable[4];
		Variable[] digby = new Variable[4];

		Variable[] guardian = new Variable[4];
		guardian[0] = new Variable(store, "durationAlgyGuardian", 30, 30);
		guardian[1] = new Variable(store, "durationBertieGuardian", 75, 75);
		guardian[2] = new Variable(store, "durationCharlieGuardian", 15, 15);
		guardian[3] = new Variable(store, "durationDigbyGuardian", 1, 1);
		Variable[] ft = new Variable[4];
		ft[0] = new Variable(store, "durationAlgyFT", 60, 60);
		ft[1] = new Variable(store, "durationBertieFT", 25, 25);
		ft[2] = new Variable(store, "durationCharlieFT", 10, 10);
		ft[3] = new Variable(store, "durationDigbyFT", 1, 1);
		Variable[] express = new Variable[4];
		express[0] = new Variable(store, "durationAlgyExpress", 2, 2);
		express[1] = new Variable(store, "durationBertieExpress", 3, 3);
		express[2] = new Variable(store, "durationCharlieExpress", 5, 5);
		express[3] = new Variable(store, "durationDigbyExpress", 1, 1);
		Variable[] sun = new Variable[4];
		sun[0] = new Variable(store, "durationAlgySun", 5, 5);
		sun[1] = new Variable(store, "durationBertieSun", 10, 10);
		sun[2] = new Variable(store, "durationCharlieSun", 30, 30);
		sun[3] = new Variable(store, "durationDigbySun", 90, 90);

		Variable[][] durations = new Variable[4][];
		...
		for (int i = 0; i < 4; i++) {
			...
			algy[i] = new Variable(store, "algy[" + i + "]", 0, 1000);
			
			// bertie[i] = new Variable(store, "bertie[" + i + "]", 15, 1000);
			bertie[i] = new Variable(store, "bertie[" + i + "]", -1000, 1000);
			...
			dom.addDom(15, 1000);
			charlie[i] = new Variable(store, "charlie[" + i + "]", dom);
			...
			digby[i] = new Variable(store, "digby[" + i + "]", new IntervalDomain(60, 1000));
			
			vars.add(algy[i]); vars.add(bertie[i]); vars.add(charlie[i]); vars.add(digby[i]);
		}

		Variable one = new Variable(store, "one", 1, 1);
		Variable[] four = new Variable[4];
		Variable[] fourOnes = { one, one, one, one };
		...
		Variable makespan = new Variable(store, "makespan", 0, 1000);
