...
public class StonesOfHeaven extends Example {
	...
	public void model() {
		...
		FDV Color[] = new FDV[4];
		FDV Country[] = new FDV[4];
		FDV Item[] = new FDV[4];
		FDV Dynasty[] = new FDV[4];

		for (int i = 0; i < 4; i++) {
			Color[i] = new FDV(store, ColorNames[i], 1, 4);
			Country[i] = new FDV(store, CountryNames[i], 1, 4);
			Item[i] = new FDV(store, ItemNames[i], 1, 4);
			Dynasty[i] = new FDV(store, DynastyNames[i], 1, 4);
