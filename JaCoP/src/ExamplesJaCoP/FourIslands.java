...
public class FourIslands extends Example {
	...
	public void model() {
		...
		String[] attractionNames = { "hotel", "ice skating rink",
				"jai alai stadium", "koala preserve" };
		...
		// arrays of variables
		FDV island[] = new FDV[4];
		FDV export[] = new FDV[4];
		FDV attraction[] = new FDV[4];
		FDV location[] = new FDV[4];

		for (int i = 0; i < 4; i++) {
			island[i] = new FDV(store, islandNames[i], 0, 3);
			export[i] = new FDV(store, exportNames[i], 0, 3);
			attraction[i] = new FDV(store, attractionNames[i], 0, 3);
			location[i] = new FDV(store, locationNames[i], 0, 3);
		...
		FDV iI3 = new FDV(store, "clue4", 1, 4);
		...
		FDV iI2 = new FDV(store, "clue5", 1, 4);
		...
		FDV iI = new FDV(store, "clue6", 1, 4);
