...
public class PerfectSquare extends Example {
	...
	public void modelBasic() {
		...
		varsX = new Variable[numberOfRectangles];
		varsY = new Variable[numberOfRectangles];
		size = new Variable[numberOfRectangles];
		...
		for (int j = numberOfRectangles - 1; j >= 0; j--) {
			...
			Variable X = new Variable(store, "x"+j, 0, masterSize-sqSize);
			
			Variable Y = new Variable(store, "y"+j, 0, masterSize-sqSize);
			
			size[j] = new Variable(store, sqSize, sqSize);
		...
		Variable[] endX = new Variable[varsX.length];
		Variable[] endY = new Variable[varsY.length];

		for (int i=0; i<varsX.length; i++) {
			endX[i] = new Variable(store, 0, masterSize);
			endY[i] = new Variable(store, 0, masterSize);
		...
		Variable limit = new Variable(store, masterSize, masterSize);

		for (int i = 0; i < masterSize - 1; i++) {
			
			ArrayList<Variable> sumList = new ArrayList<Variable>();
			
			for (int j = 0; j<varsX.length; j++) {
				Variable b = new Variable(store, 0, 1);
				store.impose(new Reified(new And(new XlteqC(varsX[j], i), new XgtC(endX[j], i)), b));
				Variable s = new Variable(store, 0, masterSize);
	...
	public void model() {
		...
		varsX = new Variable[noRectangles];
		varsY = new Variable[noRectangles];
		size = new Variable[noRectangles];

		Variable[][] rectangles = new Variable[noRectangles][4];
		...
		for (int j = noRectangles - 1; j >= 0; j--) {
			...
			Variable[] jthRectangle = {X, Y, size[j], size[j]};
		...
		store.impose(new Diff2(store, rectangles));
	...
	public void modelGeost() {
		...
		for (int j = noRectangles - 1; j >= 0; j--) {
			...
			Variable X = new Variable(store, "x"+j, 0, masterSize - sqSize);
			Variable Y = new Variable(store, "y"+j, 0, masterSize - sqSize);
			Variable S = new Variable(store, "s"+j, j, j);

			Variable startGeost = new Variable(store, "start"+j, 0, 0);
			Variable durationGeost = new Variable(store, "duration"+j, 1, 1);
			Variable endGeost = new Variable(store, "end"+j, 1, 1);
	...
	public boolean search() {
		...
		Search labelSlave1 = new DepthFirstSearch();
		SelectChoicePoint selectSlave1 = 
			new SimpleSelect(varsY, new SmallestMin(), new SmallestDomain(),
					new IndomainMin());
		...
		SelectChoicePoint selectMaster = 
			new SimpleSelect(varsX, new SmallestMin(), new SmallestDomain(),
					new IndomainMin());
	...
	public boolean search4Geost() {
		...
		Search labelMaster = new DepthFirstSearch();
		SelectChoicePoint selectMaster = 
			new SplitSelect(vars.toArray(new Variable[vars.size()]),
					null,
		//					new SmallestMin(), new SmallestDomain(),
							new IndomainMiddle());
