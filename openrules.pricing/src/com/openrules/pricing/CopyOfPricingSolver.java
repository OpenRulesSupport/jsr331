...
public class CopyOfPricingSolver {
	
	static public String VERSION = "** OpenRules Pricing Solver version 1.5.0 (build 02/17/2016) **";
...
//		for (int totalPrice = 4745; totalPrice <= 4745; totalPrice += 500) {
//		for (int totalPrice = 7210; totalPrice <= 7225; totalPrice += 5) {
			totalRuns++;
			System.out.println("\ntotal price = "+totalPrice);
			CopyOfPricingSolver pricingSolver = new CopyOfPricingSolver(numberOfItems,totalPrice,prices,quantities);
	...
	public CopyOfPricingSolver(int numberOfItems,int totalPrice, int[] prices, int[] quantities) {
...
//		this.items = items;
		...
		try {		
			...
			items = new PricingItem[numberOfItems];
...
//			Var[] vars = p.variableArray("var", 0, items.length-1, prices.length);
			...
			for (int priceIndex = 0; priceIndex < prices.length; priceIndex++) {
				Var[] itemSelectedPriceVars = new Var[items.length];
				for (int itemIndex = 0; itemIndex < items.length; itemIndex++) {
					itemSelectedPriceVars[itemIndex] = items[itemIndex].getSelectedPriceVar(priceIndex);
//					if (quantities[priceIndex] > 0)
//						p.post(itemSelectedPriceVars[itemIndex],">=",1);
				}
//				Var sumOfItemPriceVars = p.sum(itemPriceVars);
//				p.add("Sum"+i,sumOfItemPriceVars);
//				p.post(sumOfItemPriceVars,"=",selectedPriceVars[i]);
				selectedPriceVars[priceIndex] = p.sum("Slct"+priceIndex,itemSelectedPriceVars);
			}
				...
				if (solution == null && comments)
					...
					for (int i = 0; i < items.length; i++) {
						...
						for (int j = 0; j < prices.length; j++) {
							...
							String name = "i"+item.getNumber()+"p"+j;
					...
					if (success) {
						int price = solution.getValue("TotalPri");
						int revenue = solution.getValue("TotalRev");
						if (comments)
							p.log("\tTotal Price=" + price + " calcTotalPrice=" + calcTotalPrice + " Total Revenue="+revenue);
						...
						p.log("Not all items are priced. Solution ignored.");
						return null;
					}
				}
		} catch (Exception e) {
