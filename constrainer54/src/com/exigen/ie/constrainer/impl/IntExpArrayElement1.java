 ...
 * Replaced "min < values[valCounter-1]" with "min <= values[valCounter-1]"
 */
...
int[] elementDomain()
{
  ...
  for(int i=1; i < tmp.size(); i++)
  {
    ...
    // Replaced "min < values[valCounter-1]" with "min <= values[valCounter-1]" -- Roman.
    // The test here is to prevent values that we have already seen to be added
    // to the array again. If "min == values[valCounter-1]", we've clearly already
    // seen it, so we can skip.
