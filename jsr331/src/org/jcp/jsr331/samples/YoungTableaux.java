...
public class YoungTableaux extends Problem {

    int n;
    Var[][] x; // matrix
    Var[] x_a; // x as an array, for Count
    Var[] p; // partition structure.
    ...
    public static void main(String[] args) {
       ...
       int n_tmp = 4;
        ...
        YoungTableaux p = new YoungTableaux();
        p.define(n_tmp);
        p.solve();
    ...
    public void define(int n_tmp) {
        ...
        for(int i = 0; i < n; i++) {
            Var[] p_bin = new Var[n];
            for(int j = 0; j < n; j++) {
                p_bin[j] = variable("p_bin-" + j, 0, 1);
            }
            // sum all entries where x[i][j] <= n
            for(int j = 0; j < n; j++) {
                // There is no IfThenElse
                postIfThen(linear(x[i][j], "<=", n),
                           linear(p_bin[j], "=", 1)
                           );
                postIfThen(linear(x[i][j], ">", n),
                           linear(p_bin[j], "=", 0)
    ...
    public void solve() {
        ...
        while (iter.hasNext()) {
            ...
            for(int i = 0; i < n; i++) {
                System.out.print(s.getValue("p-"+i) + " ");
            }
            System.out.println("\n");
