...
public class WhoKilledAgatha {
	...
	public void solve() {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				if (i != j) {
					...
					Constraint c1 = new Linear(richer[j][i], "=", 1);
					Constraint c2 = new Linear(richer[i][j], "=", 0);
					p.postIfThen(c1, c2);
					p.postIfThen(c2, c1);
				}
			}
		...
		for (int i = 0; i < n; i++) {
			...
			Constraint c1 = new Linear(hates[i][agatha], "=", 1);
			Constraint c2 = new Linear(hates[i][charles], "=", 0);
			p.postIfThen(c1, c2);
		}
		...
		// "Who killed Agatha?"
		// p.constraint(the_victim, "=", agatha);
