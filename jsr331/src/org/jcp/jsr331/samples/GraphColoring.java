...
public class GraphColoring extends Problem {
	...
	public GraphColoring(int cliqueSize) {
		...
		log("Graph Coloring for " + cliqueSize + " cliques. " + new Date());
	}
	...
	public void define() {
		try {
			...
			vars = variableArray("vars",0, nbColors - 1, size);

			Var[][] cliques = new Var[n][n - 1];
			log("Post AllDifferent constraints");
			for (i = 0; i < n; i++) {
				for (j = 0; j < n - 1; j++) {
					int node = f(i, j);
					Var v = vars[node];
			...
			for (int k = 0; k < nbColors; k++)
				colAr[k] = k;
			log("Post Redundant Constraint (GCC)");
			cards = variableArray("cards", 0, nbColors-1, nbColors);
			postGlobalCardinality(vars,colAr,cards); 
			if (redundant_constraint) {
				int min_color_use = n / 2;
				for (int v = 0; v < cards.length; v++) {
					Var card = cards[v];
					post(card,">=",min_color_use);
				}
			}
	...
	public static void main(String[] args) {
		...
		GraphColoring p = new GraphColoring(cliqueSize);
		p.define();
		p.solve();
	}
}
