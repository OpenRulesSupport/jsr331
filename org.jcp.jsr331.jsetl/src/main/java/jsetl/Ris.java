...
public class Ris extends LSet implements Visitable {
	 ...
	 */
	public Ris(@NotNull LObject controlTerm, @NotNull LSet domain, @NotNull ConstraintClass filter, @NotNull LObject pattern, @NotNull LObject... dummyVariables){
		super();
	 ...
	 * @param filter filter of this {@code Ris}.
	 */
	public Ris(@NotNull LObject controlTerm, @NotNull LSet domain, @NotNull ConstraintClass filter){
	...
	public boolean isExpandable(){
		return expansor.canExpand(new SolverClass());
	}
	...
	@Override
	public @NotNull
	ConstraintClass forallElems(@NotNull LVar lVar, @NotNull ConstraintClass constraint){
		...
		ConstraintClass mappedConstraint = constraintMapper.mapConstraintDeeply(constraint);
		...
		ConstraintClass result = this.eq(new Ris(getControlTerm(), getDomain(), getFilter().and(mappedConstraint).and(cc), getPattern(), newDummyVariables));
		assert result != null;
		return result;
	}
	...
	protected boolean equals(@Nullable LSet lSet) {
		...
		if(lSet.isBound() && lSet.isEmpty() && this.isBound() && this.isEmpty())
			return true;
		if(this.isExpandable() && this.forceExpansion(new SolverClass()).equals(lSet))
			return true;
		else
			return false;
	...
	@NotNull
	protected ConstraintClass F(@NotNull Object controlTermInstance){
		assert controlTermInstance != null;

		Object newControlTermInstance = getNewControlTerm();
		if(!SolverClass.checkUnification(newControlTermInstance, controlTermInstance))
		...
		ConstraintClass internalConstraints = getInternalConstraints(filter);


		ConstraintClass actualFilter = filter.and(internalConstraints);
		...
		ConstraintClass generatedConstraintInstance = generateConstraintInstance(actualFilter, newControlTermInstance);
	 ...
	 */
	protected @NotNull
	ConstraintClass notF(@NotNull Object controlTermInstance){
		assert controlTermInstance != null;
		...
		ConstraintClass negatedFilter = new ConstraintNegator().negate(filter);

		ConstraintClass internalConstraints = getInternalConstraints(negatedFilter);
		ConstraintClass actualNegatedFilter = negatedFilter.and(internalConstraints);


		//Replace instances of control term with the corresponding control term instance parts
		//Replace dummy dummyVariables with new instances
		ConstraintClass generatedConstraintInstance = generateConstraintInstance(actualNegatedFilter, controlTermInstance);
	 ...
	 */
	protected @NotNull Object P(@NotNull Object controlTermInstance, @NotNull ConstraintClass constraint){
		assert controlTermInstance != null;
		...
		ConstraintClass beforeReplacementOfDummyVariables = getInternalConstraints(pattern);
	...
	/**
	 * Constructs a {@code ConstraintClass} that is the conjunction of each internal constraint in
	 ...
	 *               It usually is a {@code ConstraintClass} or a {@code LObject}.
	 * @return the constructed constraint.
	 */
	protected @NotNull
	ConstraintClass getInternalConstraints(@NotNull Object object){
		assert object != null;

		ConstraintClass internalConstraints = new ConstraintClass();
	 ...
	 * @see Ris#expand()
	 */
	protected boolean isExpandable(@NotNull SolverClass solver){
		assert solver != null;
	 ...
	 * @return the expansion of this {@code Ris}.
	 */
	protected @NotNull LSet forceExpansion(@NotNull SolverClass solver){
		assert solver != null;
	 ...
	 */
	private @NotNull
	ConstraintClass generateConstraintInstance(@NotNull ConstraintClass constraint, @NotNull Object controlTermInstance){
		assert constraint != null;
		assert controlTermInstance != null;

		ConstraintClass controlTermReplacedFilter = new ConstraintMapper(controlTerm, controlTermInstance).mapConstraintDeeply(constraint);
		ConstraintClass dummyVariablesAndControlTermReplacedFilter = dummyVariablesReplacer.replaceInConstraint(controlTermReplacedFilter);
