...
implements Visitable {
    ...
    ConstraintClass allDifferent(@NotNull Object... objects){
        Objects.requireNonNull(objects);

        ConstraintClass allDifferentConstraint = allDifferent(Arrays.asList(objects));
     ...
     */
    public static @NotNull
    ConstraintClass allDifferent(@NotNull List<?> objects){
        Objects.requireNonNull(objects);
        if(objects.stream().anyMatch(Objects::isNull))
            throw new NullPointerException("NULL VALUES NOT ALLOWED");

        ArrayList<?> lObjectsArrayList = new ArrayList<>(objects);
        ConstraintClass allDifferentConstraint = new ConstraintClass();
     ...
     * @return the constructed constraint.
     */
    public static @NotNull <T> ConstraintClass forAll(Iterable<T> iterable, LObject dummy, ConstraintClass constraint){
     ...
     */
    public static @NotNull
    ConstraintClass truec(){
        return new ConstraintClass("_true");
    }
     ...
     */
    public static @NotNull
    ConstraintClass falsec(){
        return new ConstraintClass("_false");
    }
     ...
     * with at most {@code ConstraintClass.MAX_ARGUMENTS_PER_CONSTRAINT} arguments in {@code arguments}.
     * @param name the name identifying the kind of atomic constraint to create.
     * @param arguments sequence of arguments (max {@code ConstraintClass.MAX_ARGUMENTS_PER_CONSTRAINT}).
     ...
     */
    public ConstraintClass(@NotNull String name, @NotNull Object... arguments) {
        Objects.requireNonNull(name);
        Objects.requireNonNull(arguments);

        if(arguments.length > ConstraintClass.MAX_ARGUMENTS_PER_CONSTRAINT)
            throw new IllegalArgumentException("MAX " + ConstraintClass.MAX_ARGUMENTS_PER_CONSTRAINT + " ARGUMENTS ALLOWED IN arguments");
        ...
        if(name.equals("_and")){
            ...
            if(arguments[0] instanceof ConstraintClass
                    && arguments[1] instanceof ConstraintClass){

                addAll(((ConstraintClass)arguments[0]));
                addAll(((ConstraintClass)arguments[1]));
            }
            else
     ...
     */
    protected ConstraintClass(@NotNull ConstraintClass constraint) {
     ...
     * @param arguments sequence of arguments (max {@code ConstraintClass.MAX_ARGUMENTS_PER_CONSTRAINT}).
     */
    protected ConstraintClass(int constraintKindCode, @NotNull Object... arguments) {
     ...
     */
    protected ConstraintClass(@NotNull AConstraint aConstraint) {
    ...
    /**
     * Creates and returns a clone of this constraint (the same as {@code new ConstraintClass(this)}.
     * @return a clone of this constraint.
     */
    @Override
    public @NotNull
    ConstraintClass clone() {
    ...
    public boolean equals(@Nullable Object other) {
        ...
        if(other instanceof ConstraintClass){
            ConstraintClass otherConstraint = (ConstraintClass) other;
    ...
    public @Nullable Object getArg(int i) {
        ...
        if(size == 1)
            ...
            if (i == 1){   // the first conjunct: c1
                ...
                ConstraintClass first = new ConstraintClass(argumentAConstraint);
                return first;
            }
            if (i == 2){    // the rest of the conjunction: c2 AND ... AND cn
                ConstraintClass rest = new ConstraintClass();
     ...
     * @see ConstraintClass#solve()
     */
    public boolean test() { 
        ConstraintClass cCopy = this.clone();
        SolverClass localSolver = new SolverClass();
    ...
    /**
     * Similar to the method {@code ConstraintClass#toString()} but gives a different string representation: one of the form
    ...
    /**
     * Raises an exception {@code Fail}. This exception is handled by the {@code SolverClass} which
     ...
     */
    public @NotNull
    ConstraintClass add(@NotNull ConstraintClass constraint) {
        Objects.requireNonNull(constraint);
     ...
     */
    public @NotNull
    ConstraintClass impliesTest(@NotNull ConstraintClass constraint) {
        Objects.requireNonNull(constraint);

        ConstraintClass implicationTest = new ConstraintClass(Environment.impliesTestCode, this, constraint);
     ...
     */
    public @NotNull
    ConstraintClass notTest() {
        ConstraintClass notTest = new ConstraintClass(Environment.notTestCode, this);
        return notTest;
    }
     ...
     */
    public @NotNull
    ConstraintClass or(@NotNull ConstraintClass constraint) {
        Objects.requireNonNull(constraint);

        ConstraintClass disjunction = new ConstraintClass(Environment.orCode, this, constraint);
        return disjunction;
    }
     ...
     */
    public @NotNull
    ConstraintClass orTest(@NotNull ConstraintClass constraint) {
        Objects.requireNonNull(constraint);

        ConstraintClass orTest = new ConstraintClass(Environment.orTestCode, this, constraint);
        return orTest;
    }
     ...
     */
    protected void addAll(@NotNull ConstraintClass constraint) {
