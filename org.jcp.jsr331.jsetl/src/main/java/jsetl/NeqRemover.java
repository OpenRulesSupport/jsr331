...
class NeqRemover {
     ...
     */
    NeqRemover(@NotNull SolverClass solver){
        assert solver != null;
        this.solver = solver;
    }
    ...
    void removeNeq(){
        ...
        for(AConstraint inequality : inequalities){
            if(rewriteNeeded(inequality, setsInUnsafeSolvedForms)){
                ConstraintClass rewrittenInequality = getRewrittenInequality(inequality);
     ...
     */
    @NotNull
    ConstraintClass getRewrittenInequality(@NotNull AConstraint neq){
        assert neq != null;
        ...
        ConstraintClass newConstraintFirst = new ConstraintClass(Environment.inCode, newLVar, neq.argument1)
                        .and(new ConstraintClass(Environment.ninCode, newLVar, neq.argument2));

        ConstraintClass newConstraintSecond =
                new ConstraintClass(Environment.inCode, newLVar, neq.argument2)
                        .and(new ConstraintClass(Environment.ninCode, newLVar, neq.argument1));

        ConstraintClass newConstraintThird =
                new ConstraintClass(Environment.eqCode, neq.argument1, LSet.empty())
                        .and(new ConstraintClass(Environment.neqCode, neq.argument2, LSet.empty()));

        ConstraintClass rewritten = newConstraintFirst.or(newConstraintSecond).or(newConstraintThird);
