...
public class MpsGeneratorReal {
	...
	public MpsGeneratorReal(Problem problem, int objectiveDirection,
		...
		for (int i = 0; i < constraints.length; i++) {
			...
			rowTypes[i] = getRowType(c.getOper());
	 ...
	 * 
	 * @param file
	 *            the location of the .mps format representation
	 */
	...
	private void writeRows() {
		...
		if (!problem.isDerivedVar(objectiveVar))
			throw new RuntimeException("Objective variable "
	...
	private void writeColumns(int thirdColStart) {
		...
		// markerCount++;
		// }
		String str = "    " + objectiveVar.getName();
		str = rightPadWhitespace(str, thirdColStart);
		write("\n" + str);
