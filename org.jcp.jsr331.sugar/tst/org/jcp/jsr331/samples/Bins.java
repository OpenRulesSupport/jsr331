...
public class Bins {
	...
	public Bins(int numberOfBins) throws Exception {
		...
		csp = ProblemFactory.newProblem("ConfigureBins " + numberOfBins);
...
//				bin1.type.le(bin2.type);
		...
		for (int j = 0; j < components.length; j++) {
			...
			try {
				...
				Var sumOfCounts = csp.sum(binCounts);
				csp.post(sumOfCounts, "=", demand[j]);
			} catch (Exception e) {
...
//		solver.trace(counts,PropagationEvent.ANY);
	...
	class Bin {
		...
		Bin(Problem csp,int binId) {
			...
			Var sumCounts = csp.sum(counts);
			csp.post(sumCounts, "<=", capacity);
...
//			pb.element(binCapacities, type, ">=", sumOfCounts);
			...
			// red contains at most 1 of wood
			c1 = csp.linear(type,"=",red);
			c2 = csp.linear(counts[wood],"<=",1);
			c1.implies(c2).post();

			// green contains at most 2 of wood
			c1 = csp.linear(type,"=",green); //????
			c2 = csp.linear(counts[wood],"<=",2);
			c1.implies(c2).post();

			// red can contain glass, wood, copper
			c1 = csp.linear(type,"=",red);
			c2 = csp.linear(counts[plastic],"=",0);
			c3 = csp.linear(counts[steel],"=",0);
			c1.implies(c2.and(c3)).post();

			// blue can contain glass, steel, copper
			c1 = csp.linear(type,"=",blue);
			c2 = csp.linear(counts[plastic],"=",0);
			c3 = csp.linear(counts[wood],"=",0);
			c1.implies(c2.and(c3)).post();

			// green can contain plastic, wood, copper
			c1 = csp.linear(type,"=",green);
			c2 = csp.linear(counts[glass],"=",0);
			c3 = csp.linear(counts[steel],"=",0);
			c1.implies(c2.and(c3)).post();

			// wood requires plastic
			c1 = csp.linear(counts[wood],">",0);
			c2 = csp.linear(counts[plastic],">",0);
			c1.implies(c2).post();

			// glass exclusive copper
			c1 = csp.linear(counts[glass],"=",0);
			c2 = csp.linear(counts[copper],"=",0);
			c1.or(c2).post();

			// copper exclusive plastic
			c1 = csp.linear(counts[copper],"=",0);
			c2 = csp.linear(counts[plastic],"=",0);
			c1.or(c2).post();
		}
...
Bin#2 (red): glass=1
Bin#3 (blue): steel=1
Bin#4 (green): plastic=1 wood=1
Bin#5 (green): plastic=1 wood=2
*/
